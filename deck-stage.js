...
(() => {
  ...
  class DeckStage extends HTMLElement {
    ...
    _fit() {
      ...
      // Scale to the host's own box (not the window) so an outer rule that
      // shrinks <deck-stage> — e.g. reserving a strip for custom nav — is
      // honoured instead of clipping the slide bottom behind it.
      const hostH = this.clientHeight || window.innerHeight;
      const hostW = this.clientWidth || window.innerWidth;
      const vw = hostW - rw;
      const vh = hostH;
